...
option_dict={'-cores':"32",'-paired':"1", '-exclude':"",'-ref':"", '-target':"1", '-out':"RSEM_results", '-build_index':"2", '-parsing_only':"2"}
help="""
...
1) run all steps: use all options
   example: python Automated_RSEM-Bowtie2_multi-processing_py3_v2.0.py -build_index 1 \\
            -include .fq -exclude .fa -paired 1 -parsing_only 2 -target 1

2) run RSEM and parse RSEM results: set '-build_index' option as '2' and use all options
   example: python Automated_RSEM-Bowtie2_multi-processing_py3_v2.0.py -build_index 2 \\
            -ref genome.fa -include .fq -exclude .fa -paired 1 -parsing_only 2 -target 1

3) only parse RSEM results: set '-parsing_only' option as '1' and use '-target' option.
   example: python Automated_RSEM-Bowtie2_multi-processing_py3_v2.0.py -parsing_only 1 -target 1
